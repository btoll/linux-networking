...
then
    ...
    container_ip=$(ip -n "$NAMESPACE" -br a | awk '/ceth/ {split($3, a, "/"); print a[1]}')
    # `@tsv` ensures that values containing spaces don't break the `read` command.
    read -r ip port protocol < <(jq -r '[.ip, .port, .protocol] | @tsv' "$SERVICES_DIR/$SERVICE_NAME.json")
    ...
    do
        ...
        # It does the same job as the KUBE-MARK-MASQ rules in `kube-proxy`.
